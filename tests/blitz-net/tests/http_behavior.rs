...
async fn injects_user_agent_header() {
    ...
    let ua = requests[0]
        ...
        .unwrap_or_default();
    let version = ua
        .strip_prefix("Mozilla/5.0 (compatible; Blitz/")
        .and_then(|s| s.split(';').next())
        .unwrap_or_else(|| {
            panic!(
                "user-agent missing 'Mozilla/5.0 (compatible; Blitz/<version>;' prefix, got: '{ua}'"
            )
        });
    assert!(
        version.chars().next().is_some_and(|c| c.is_ascii_digit()),
        "version segment should start with a digit, got: '{version}'"
    );
    assert!(
        version.contains('.'),
        "version segment should look like semver, got: '{version}'"
    );
    assert!(
        ua.ends_with("+https://github.com/DioxusLabs/blitz)"),
        "user-agent should end with contact URL, got: '{ua}'"
    );
}
...
async fn net_provider_trait_fetch_delivers_bytes_to_handler() {
    ...
    let result = rx.await.expect("handler should be called");
    let (_, bytes) = result.expect("handler result should be ok");
...
async fn net_provider_trait_fetch_skips_handler_on_error() {
    ...
    let url = make_url(&server.uri());
    let (tx, mut rx) = tokio::sync::oneshot::channel::<Result<(String, Bytes), String>>();
    let handler = CaptureHandler(tx);
...
async fn shared_returns_arc_dyn_netprovider() {
    ...
    let result = rx.await.expect("shared provider should work");
    assert!(result.is_ok());
}
...
async fn provider_error_display_renders_each_variant() {
    ...
    assert!(matches!(err, ProviderError::DataUrl(_)));
    assert!(!err.to_string().is_empty());
    ...
    assert!(matches!(err, ProviderError::DataUrlBase64(_)));
    assert!(!err.to_string().is_empty());
    ...
    // Workspace feature unification can enable blitz-net's `cache` feature even
    // when this test crate doesn't ask for it (e.g., apps/browser's defaults
    // pull it in), wrapping reqwest errors in reqwest-middleware. The variant
    // isn't always in scope here, so assert on the display string instead.
    let display = err.to_string();
    assert!(
        display.starts_with("reqwest error:") || display.starts_with("reqwest middleware error:"),
        "expected reqwest/middleware error display, got: {display}"
    );
    ...
    let err = provider
        ...
        .await
        .expect_err("404");
    assert!(matches!(err, ProviderError::HttpStatus { .. }));
    let display = err.to_string();
    assert!(
        display.contains("404"),
        "display should contain status: {display}"
    );
    assert!(
        display.contains(&server.uri()),
        "display should contain url: {display}"
    );

    let abort_err = ProviderError::Abort;
    assert!(abort_err.to_string().contains("aborted"));
}
